...
int setting_envvar(const char *const arg)
{
    ...
    int i = 0;
    while (*ptr && (i < MAX_ENV_NAME_LENGTH - 1) && (isalnum(*ptr) || *ptr == '_'))
        var[i++] = *ptr++;
    if (*ptr != '=')
        return 0;
    ptr++;
...
int call_command(int argc, char **argv, int *const use_pipe, const int pipe_used, const int is_piped)
{
    ...
    *use_pipe = is_piped;
    ...
    {
        int count = 0;
        while (count < HISTORY_SIZE && HISTORY[count][0])
            count++;
        ...
        return_code = our_cat(cmd_argc, cmd_argv);
    else if (strcmp(cmd_argv[0], "cd") == 0)
        return_code = our_cd(cmd_argc, cmd_argv);
    else if (strcmp(cmd_argv[0], "chmod") == 0)
        return_code = our_chmod(cmd_argc, cmd_argv);
    else if (strcmp(cmd_argv[0], "chown") == 0)
        return_code = our_chown(cmd_argc, cmd_argv);
    else if (strcmp(cmd_argv[0], "cp") == 0)
        return_code = our_cp(cmd_argc, cmd_argv);
    else if (strcmp(cmd_argv[0], "ls") == 0)
        return_code = our_ls(cmd_argc, cmd_argv);
    else if (strcmp(cmd_argv[0], "mkdir") == 0)
        return_code = our_mkdir(cmd_argc, cmd_argv);
    else if (strcmp(cmd_argv[0], "mv") == 0)
        return_code = our_mv(cmd_argc, cmd_argv);
    else if (strcmp(cmd_argv[0], "rm") == 0)
        return_code = our_rm(cmd_argc, cmd_argv);
    else if (strcmp(cmd_argv[0], "touch") == 0)
        return_code = our_touch(cmd_argc, cmd_argv);
    else
    {
        pid_t pid = fork();
        if (pid == 0)
...
int parse_commands(int argc, char **argv)
{
    ...
    while (end < argc)
    {
        ...
        // Run parsed command, capture its output inside our pipe and wait for it to finish
            ...
            return_code = call_command(end - start, &argv[start], &use_pipe, pipe_used, 1);
    ...
    *argv = malloc(argv_capacity * sizeof(char *));
    ...
    while (*ptr)
    {
        ...
        while (*ptr && *ptr != end && !(end == ' ' && isspace((unsigned char)*ptr)))
        {
            ...
            if ((len >= MAX_ENV_NAME_LENGTH) || !(isalnum(*ptr) || *ptr == '_' || *ptr == '='))
                is_name = 0;
            ...
            else if (*ptr == '$' && end != '\'')
            {
                ...
                if (*ptr == '(')
                {
                    ...
                    while (*ptr && *ptr != ')')
                    {
                        ...
                        if (sub_i < MAX_LINE_LENGTH - 1)
                            sub_command[sub_i++] = *ptr++;
                    }
                    if (*ptr == ')')
                        ptr++;
                ...
                else if (*ptr == '{' || isalnum(*ptr) || *ptr == '_')
                {
                    ...
                    if (sub_bracket)
                        ptr++;
                    while (*ptr && (sub_i < MAX_ENV_NAME_LENGTH - 1) && (isalnum(*ptr) || *ptr == '_'))
                        sub_varname[sub_i++] = *ptr++;
                    if (sub_bracket)
                        while (*ptr && *ptr != '}')
                            ptr++;
                    if (*ptr == '}')
...
int main(int argc, char *argv[])
{
    ...
    while (1)
    {
        ...
        if (DEBUG)
            printf("Last command return code: %d\n", return_code);
        fflush(stdout);
