 ...
 * @see fprintf
 */
void __echo_print_usage(char *program_name) {
 ...
 * @return
 * @see strcmp, __echo_print_usage, exit
 */
void __echo_parse_arguments(int argc, char *argv[]) {
    // TODO: Implement the parse_arguments function
    for (int i = 1; i < argc; i++) {
        // Check if the argument is -h or --help
        if (strcmp(argv[i], "-h") == 0 || strcmp(argv[i], "--help") == 0) {
            // Print the usage and exit
            __echo_print_usage(argv[0]);
            exit(0);
        }
    }
}
 ...
 * @see __echo_parse_arguments
 */
int our_echo(int argc, char *argv[]) {
    // Parse the arguments
    __echo_parse_arguments(argc, argv);
